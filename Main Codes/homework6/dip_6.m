...
figure('NumberTitle','off','Name','gaussÔëÉù')
...
figure('NumberTitle','off','Name','gaussÔëÉù»Ö¸´')
subplot(3,2,1);imshow(i_origin);title('Ô­Í¼');
subplot(3,2,2);imshow(i_gaussnoise1);title('gaussÔëÉùav=0,std=0.1');
subplot(3,2,3);imshow(i_gauss_gauss1);title('gaussÂË²¨Æ÷ 3x3');
subplot(3,2,4);imshow(i_gauss_gauss2);title('gaussÂË²¨Æ÷ 5x5');
subplot(3,2,5);imshow(i_gauss_median1);title('ÖĞÖµÂË²¨Æ÷ 3x3');
subplot(3,2,6);imshow(i_gauss_median2);title('ÖĞÖµÂË²¨Æ÷ 5x5');
...
figure('NumberTitle','off','Name','½·ÑÎÔëÉù')
subplot(2,2,1);imshow(i_origin);title('origin');
subplot(2,2,2);imshow(i_saltpepper);title('½·ÑÎÔëÉùa=b=0.1');
subplot(2,2,3);imshow(i_pepper);title('ºú½·ÔëÉùa=0.1,b=0');
subplot(2,2,4);imshow(i_salt);title('°×ÑÎÔëÉùa=0,b=0.1');
...
figure('NumberTitle','off','Name','½·ÑÎÔëÉù»Ö¸´')
subplot(3,2,1);imshow(i_origin);title('Ô­Í¼');
subplot(3,2,2);imshow(i_saltpepper);title('½·ÑÎÔëÉùa=b=0.1');
subplot(3,2,3);imshow(i_saltpepper_gauss1);title('gaussÂË²¨Æ÷ 3x3');
subplot(3,2,4);imshow(i_saltpepper_gauss2);title('gaussÂË²¨Æ÷ 5x5');
subplot(3,2,5);imshow(i_saltpepper_median1);title('ÖĞÖµÂË²¨Æ÷ 3x3');
subplot(3,2,6);imshow(i_saltpepper_median2);title('ÖĞÖµÂË²¨Æ÷ 5x5');
...
figure('NumberTitle','off','Name','·´Ğ³²¨¾ùÖµÂË²¨')
subplot(3,3,1);imshow(i_origin);title('Ô­Í¼');
subplot(3,3,2);imshow(i_pepper);title('ºú½·ÔëÉù');
subplot(3,3,3);imshow(i_salt);title('°×ÑÎÔëÉù');
subplot(3,3,4);imshow(i_origin);title('Ô­Í¼');
subplot(3,3,5);imshow(ic1_pos);title('ºú½·ÔëÉù Q=2');
subplot(3,3,6);imshow(ic2_pos);title('°×ÑÎÔëÉù Q=2');
subplot(3,3,7);imshow(i_origin);title('Ô­Í¼');
subplot(3,3,8);imshow(ic1_neg);title('ºú½·ÔëÉù Q=-2');
subplot(3,3,9);imshow(ic2_neg);title('°×ÑÎÔëÉù Q=-2');
...
figure('NumberTitle','off','Name','Î¬ÄÉÂË²¨Æ÷')
subplot(2,3,1);imshow(i_origin);title('Ô­Í¼');
subplot(2,3,2);imshow(i_blur);title('Ä£ºı');
subplot(2,3,3);imshow(i_blur_done);title('Ä£ºı+ÔëÉù');
subplot(2,3,4);imshow(i_origin);title('Ô­Í¼');
subplot(2,3,5);imshow(i_wnrecover_done);title('Î¬ÄÉÂË²¨Æ÷');
subplot(2,3,6);imshow(i_reg);title('Ô¼Êø×îĞ¡¶ş³Ë·¨');
...
exsize=floor(masksize/2);   %¸÷·½ÏòÀ©Õ¹´óĞ¡
Imgex=padarray(Img,[exsize,exsize],'replicate','both'); %À©Õ¹Í¼Æ¬
[m,n]=size(Img);
Img_out=Img;    %½«Img_out×¼±¸ÎªºÍImgÏàÍ¬µÄsize
for i=1:m
    for j=1:n
        neighbor=Imgex(i:i+masksize-1,j:j+masksize-1);  %½ØÈ¡ÁÚÓò
        Img_out(i,j)=median(neighbor(:));   %ÖĞÖµÂË²¨
    end
end
end
%·´Ğ³²¨¾ùÖµÂË²¨£º
